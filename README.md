...
#### 日志原则：只记错误（touch.log）
`touch.log` **只保留错误/异常**：点击成功不再逐条写日志（以前每次点击 2~3 行，挂机几小时就把
500 行额度占满）。判定"点击是否被系统接收"仍靠**自身回显**（下发的事件会再流经 HID 事件系统、
被本进程常驻监听 client 收到，与肉手触摸同一条通路），但现在只在**没收到回显**时告警：
```
[touch] ⚠ 点击 #12 未收到系统回显 (100.0,200.0) 通道=自动: 事件很可能未被 HID 事件系统接收(这次点击可能没生效)
```
- 没有这类告警 = 每次点击都被系统接收了；
- 出现该告警 = 这一击可能没生效（回显通常 1~10ms 到达，判定会延迟 0.3s 复核，避免极短点击误报）；
- 正常路径信息请**主动查询**：`log(touch.status())`（含 senderID/来源、client 状态、通道、
  已下发次数、最近一击回显次数）；
- 其它保留在日志里的都是失败原因：`⚠ 直发失败: IOHIDEventSystemClient 未创建`、
  `本应用点击(AX)失败 …`、`(进程内 UIControl)失败 …`、`IOHID 直发不可用…进入本应用点击模式`。
- 回显依赖常驻的监听 client，因此程序**不会**再像旧版那样"拿到真实 senderID 就释放监听"。
- 日志容量：`touch.log` / `debug.log` **各自最多 500 行**（超出裁剪为最新 500 行），
  内存与设置页同样保留 500 行；另有 `[HUD] 宿主状态`、tap 坐标映射、senderID 探测、
  触摸/回显监听、后台任务续期、音量键停止步骤、重复 toast 等一批日志已删除/限流/改为 NSLog。

#### 同一时刻只允许一个 Lua 脚本
启动第二个脚本会被拒绝，并弹出提示 **"其他脚本正在运行，请先停止"**（若旧脚本正在停止中，
提示为"上一个脚本正在停止中，请稍候再启动"）。覆盖脚本列表/网页"开始运行"、悬浮球 ▶、
音量键运行、`/task?cmd=start`、API 上传后自动运行等全部入口；判定基于"派发前就抢占的运行位"，
因此"已派发还没开跑"的窗口期再点一次也会被拒绝，不会出现"点了停止却自己跑起来"。
